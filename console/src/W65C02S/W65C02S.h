...
// Sampling behaviour.
// -----------------------------------------------------------------------------
// The clock reference for sampling is PHI2O (chip pin 39), the buffered in-phase
// phase-2 clock output. The 6502 puts a valid address on the bus during PHI2-low
// and latches/presents data around the PHI2 rising edge; the whole cycle's bus
// contents are valid and stable while PHI2 is HIGH. We therefore sample one
// snapshot per clock cycle on the RISING edge of PHI2O. Set to FALLING if your
// build prefers it. PHI2O (pin 39) must be on an interrupt-capable Mega pin
// (2, 3, 18, 19, 20, 21).
